...
export default function SpecialsTable({ especiales, totales }) {
  return (
    <div>
      {totales.descCalculadoEspeciales?.toFixed(2) > 0 ? (
        <Table>
          <thead>
            <tr>
              <th>Nombre</th>
            ...
            {especiales.map((esp) => {
              return (
                <tr>
                  ...
                  <td>{`${(esp.precioDescuentoFijo * esp.cantProducto).toFixed(
                    2
                  )} Bs.`}</td>
                </tr>
              );
